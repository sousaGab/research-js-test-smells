"""
RQ5 Controller — Refactoring Success Rate (RSR) analysis with multi-metric coverage guardrails.

What is the overall success rate of LLM-generated test refactorings when coverage
is evaluated with all available coverage dimensions?

A refactoring attempt is considered SUCCESSFUL iff ALL three conditions hold:
  1. smell_removed = 1
  2. behavior_preserved = 1
  3. no coverage degradation beyond -0.5 pp in any coverage metric
...
Derived indicators:
  ...
  coverage_violation = 1 if any coverage metric degrades below -0.5 pp, else 0
...
Aggregations produced:
  ...
  Table F — Coverage metric breakdown

Inclusion scope:
  Only experiments that have an after-phase test_results row (tr_a.id IS NOT NULL).

Endpoints:
  GET /api/rq5/summary  – all aggregated data for the dashboard
  GET /api/rq5/export   – CSV download, table= param selects which dataset
"""
...
router = APIRouter(prefix="/api/rq5", tags=["RQ5"])
...
def _coverage_delta_expr(metric_column: str) -> str:
    return f"COALESCE(tr_a.{metric_column}, 0) - COALESCE(tr_b.{metric_column}, 0)"


COVERAGE_DELTA_STATEMENTS_EXPR = _coverage_delta_expr("coverage_statements")
COVERAGE_DELTA_BRANCHES_EXPR = _coverage_delta_expr("coverage_branches")
COVERAGE_DELTA_FUNCTIONS_EXPR = _coverage_delta_expr("coverage_functions")
COVERAGE_DELTA_LINES_EXPR = _coverage_delta_expr("coverage_lines")

COVERAGE_VIOLATION_EXPR = f"""
    CASE WHEN
        ({COVERAGE_DELTA_STATEMENTS_EXPR}) < -0.5
        OR ({COVERAGE_DELTA_BRANCHES_EXPR}) < -0.5
        OR ({COVERAGE_DELTA_FUNCTIONS_EXPR}) < -0.5
        OR ({COVERAGE_DELTA_LINES_EXPR}) < -0.5
    THEN 1 ELSE 0 END
"""
...
def rq5_summary(
    ...
    """Returns aggregated data needed to render the RQ5 dashboard."""
    session = _session()
    try:
        ...
        total_experiments = session.execute(text("SELECT COUNT(*) FROM experiments")).scalar() or 0
        ...
        failure_causes = [
            ...
            },
            {
                ...
                "label": "Coverage degradation (< −0.5 pp in any metric)",
                "count": coverage_cnt,
        ...
        coverage_metrics = []
        for metric_name, delta_expr in [
            ("statements", COVERAGE_DELTA_STATEMENTS_EXPR),
            ("branches", COVERAGE_DELTA_BRANCHES_EXPR),
            ("functions", COVERAGE_DELTA_FUNCTIONS_EXPR),
            ("lines", COVERAGE_DELTA_LINES_EXPR),
        ]:
            violation_expr = f"CASE WHEN ({delta_expr}) < -0.5 THEN 1 ELSE 0 END"
            metric_row = session.execute(text(f"""
                SELECT
                    SUM({violation_expr}) AS violations,
                    AVG({delta_expr})     AS avg_delta
                {SMELL_JOIN}
                {where}
            """), params).fetchone()
            coverage_metrics.append({
                "metric": metric_name,
                "violations": int(metric_row[0] or 0),
                "avg_delta": round(float(metric_row[1]), 3) if metric_row[1] is not None else None,
                "threshold_pp": -0.5,
            })
...
_EXPORT_TABLES = {"raw", "overall", "by_prompt", "by_model", "model_matrix", "failure_causes", "coverage_breakdown"}
        ...
        if table == "raw":
            rows = session.execute(text(f"""
                SELECT
                    ...
                    ({COVERAGE_DELTA_STATEMENTS_EXPR})                             AS delta_coverage_statements,
                    ({COVERAGE_DELTA_BRANCHES_EXPR})                               AS delta_coverage_branches,
                    ({COVERAGE_DELTA_FUNCTIONS_EXPR})                              AS delta_coverage_functions,
                    ({COVERAGE_DELTA_LINES_EXPR})                                  AS delta_coverage_lines,
            ...
            writer.writerow([
                ...
                "smell_removed", "behavior_preserved",
                "delta_coverage_statements", "delta_coverage_branches",
                "delta_coverage_functions", "delta_coverage_lines",
            ...
            data = rq5_summary(smell_type, ai_model_version, prompting_approach)
            ...
            data = rq5_summary(smell_type, ai_model_version, prompting_approach)
            prompts = sorted({p for row in data["model_matrix"] for p in row["by_prompt"].keys()})
            writer.writerow(["model"] + prompts + ["overall_attempts", "overall_successes", "overall_success_rate"])
            for row in data["model_matrix"]:
                cells = []
                for p in prompts:
                    bp = row["by_prompt"].get(p)
                    cells.append(f"{bp['success_rate']}% ({bp['successes']}/{bp['n']})" if bp else "—")
                writer.writerow([row["model"]] + cells + [row["overall_n"], row["overall_successes"], row["overall_success_rate"]])

        elif table == "failure_causes":
            data = rq5_summary(smell_type, ai_model_version, prompting_approach)
            writer.writerow(["cause", "label", "count", "pct_of_failures"])
            for row in data["failure_causes"]:
                writer.writerow([row["cause"], row["label"], row["count"], row["pct_of_failures"]])

        elif table == "coverage_breakdown":
            data = rq5_summary(smell_type, ai_model_version, prompting_approach)
            writer.writerow(["metric", "violations", "avg_delta_pp", "threshold_pp"])
            for row in data["coverage_breakdown"]:
                writer.writerow([row["metric"], row["violations"], row["avg_delta"], row["threshold_pp"]])

        filename = f"rq5_{table}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
