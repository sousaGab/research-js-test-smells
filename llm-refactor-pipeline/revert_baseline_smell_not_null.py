#!/usr/bin/env python3
"""
Revert migration: make baseline_smell_id NOT NULL again.

Since we're now creating baseline_smells automatically from study_smells,
we can make baseline_smell_id NOT NULL again for data integrity.
"""
...
try:
    print("\n🔄 Reverting migration: making baseline_smell_id NOT NULL...")
    
    # Step 1: Create new table with baseline_smell_id as NOT NULL
    ...
    # Step 2: Copy data from old table (only rows with baseline_smell_id)
    print("  [2/6] Copying existing data...")
    cursor.execute("""
        INSERT INTO experiments_new 
        SELECT * FROM experiments WHERE baseline_smell_id IS NOT NULL
    """)
